...
function MenuSearchbar({ toggleMenu }) {
  ...
  const handleProductClick = (product) => {
    ...
    setShowDropdown(false)
    setSelectedIndex(-1)
    navigate(`/products/${product.name.replaceAll(" ", "-")}`)
    toggleMenu()
  }
  ...
  return (
    <div className="relative px-3 mt-1" ref={dropdownRef}>
      ...
      {showDropdown && searchProducts && searchProducts.length > 0 && (
        <div className="absolute z-[10000] top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg max-h-[400px] overflow-y-auto animate-fadeIn ">
      ...
      {isError && !isLoading && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 text-center animate-fadeIn">
          <p className="text-red-500">{error || 'Error searching products'}</p>
        </div>
      )}
    </div>
  )
